...
export const viewModeStorageKey = "carnalys:view-mode:v1";
...
export function setStoredViewMode(mode: VehicleViewMode) {
  cachedSnapshot = mode;
  try {
...
export function useViewMode(): [VehicleViewMode, (mode: VehicleViewMode) => void] {
  ...
  const setViewMode = useCallback((mode: VehicleViewMode) => setStoredViewMode(mode), []);
